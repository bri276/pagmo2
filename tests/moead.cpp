...
TEST(moead_test, moead_algorithm_construction)
{
    moead uda{10u, "grid", "tchebycheff", 20u, 1., 0.5, 20., 0.9, 2u, true, 23u};
    EXPECT_TRUE(uda.get_verbosity() == 0u);
    EXPECT_TRUE(uda.get_seed() == 23u);
    EXPECT_TRUE((uda.get_log() == moead::log_type{}));

    // Check the throws
    // Wrong weight generation type
    EXPECT_THROW((moead{10u, "typo", "tchebycheff", 20u, 1., 0.5, 20., 0.9, 2u, true, 23u}), invalid_parameter_error);
    ...
    // Wrong realb
    EXPECT_THROW((moead{10u, "grid", "tchebycheff", 20u, 1., 0.5, 20., 1.1, 2u, true, 23u}), invalid_parameter_error);
    EXPECT_THROW((moead{10u, "grid", "tchebycheff", 20u, 1., 0.5, 20., -0.34, 2u, true, 23u}), invalid_parameter_error);
    // Wrong neighbours
    EXPECT_THROW((moead{10u, "grid", "tchebycheff", 1u, 1., 0.5, 20., 0.9, 2u, true, 23u}), invalid_parameter_error);
}
...
TEST(moead_test, moead_evolve_test)
{
    ...
    EXPECT_THROW(moead{10u}.evolve(population{problem{mo_con{}}, 20u}), incompatible_problem_error);
    // Stochastic problem
...
TEST(moead_test, moead_setters_getters_test)
{
...
TEST(moead_test, moead_serialization_test)
{
    // Make one evolution
    problem prob{zdt{1u, 30u}};
