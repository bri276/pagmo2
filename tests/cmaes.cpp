...
TEST(cmaes_test, cmaes_algorithm_construction)
{
...
TEST(cmaes_test, cmaes_evolve_test)
{
    {
    ...
    EXPECT_NO_THROW(cmaes{10u}.evolve(population{problem{inventory{}}, 15u}));
}

TEST(cmaes_test, cmaes_setters_getters_test)
{
    cmaes user_algo{10u, -1, -1, -1, -1, 0.5, 1e-6, 1e-6, false, false, 23u};
    ...
    EXPECT_TRUE(user_algo.get_seed() == 23u);
    EXPECT_TRUE(user_algo.get_name().find("CMA-ES") != std::string::npos);
    EXPECT_TRUE(user_algo.get_extra_info().find("cmu") != std::string::npos);
...
TEST(cmaes_test, cmaes_serialization_test)
{
    // Make one evolution
    problem prob{rosenbrock{25u}};
...
TEST(cmaes_test, cmaes_memory_test)
{
