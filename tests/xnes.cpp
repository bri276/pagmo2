...
TEST(xnes_test, xnes_algorithm_construction)
{
    ...
    EXPECT_TRUE((user_algo.get_log() == xnes::log_type{}));
}
...
TEST(xnes_test, xnes_evolve_test)
{
    {
    ...
    EXPECT_NO_THROW(xnes{10u}.evolve(population{problem{inventory{}}, 15u}));
}

TEST(xnes_test, xnes_setters_getters_test)
{
    ...
    EXPECT_TRUE(user_algo.get_name().find("xNES") != std::string::npos);
    EXPECT_TRUE(user_algo.get_extra_info().find("eta_mu") != std::string::npos);
    EXPECT_TRUE(user_algo.get_extra_info().find("auto") != std::string::npos);
    EXPECT_TRUE(user_algo2.get_extra_info().find("auto") == std::string::npos);
    EXPECT_NO_THROW(user_algo.get_log());
}

TEST(xnes_test, xnes_serialization_test)
{
    // Make one evolution
    problem prob{rosenbrock{25u}};
...
TEST(xnes_test, xnes_memory_test)
{
    ...
    EXPECT_NEAR(std::get<5>(log[0]), std::get<5>(log2[1]), 1e-8);
    EXPECT_NEAR(std::get<4>(log[0]), std::get<4>(log2[1]), 1e-8);
    EXPECT_NEAR(std::get<3>(log[0]), std::get<3>(log2[1]), 1e-8);
    EXPECT_NEAR(std::get<2>(log[0]), std::get<2>(log2[1]), 1e-8);
