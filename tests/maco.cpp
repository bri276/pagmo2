...
TEST(maco_test, maco_algorithm_construction)
{
    maco user_algo{1u, 63u, 1.0, 1u, 7u, 10000u, 0., false, 23u};
    EXPECT_NO_THROW(maco{});
...
TEST(maco_test, maco_setters_getters_test)
{
    maco user_algo{1u, 63u, 1.0, 1u, 7u, 10000u, 0., false, 23u};
    ...
    EXPECT_TRUE(user_algo.get_name().find("MHACO: Multi-objective Hypervolume-based Ant Colony Optimization")
                != std::string::npos);
    EXPECT_TRUE(user_algo.get_extra_info().find("Verbosity") != std::string::npos);
}

// Integer test
TEST(maco_test, maco_zdt5_test)
{
...
TEST(maco_test, maco_serialization_test)
{
    // Make one evolution
...
TEST(maco_test, bfe_usage_test)
{
    ...
    EXPECT_TRUE(algo1.extract<maco>()->get_log() == algo2.extract<maco>()->get_log());
}

// We now introduce some tests for coverage purposes:
TEST(maco_test, miscellanea_tests)
{
...
TEST(maco_test, out_of_bounds_test)
{
